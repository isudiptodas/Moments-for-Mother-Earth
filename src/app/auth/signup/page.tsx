...
function page() {
    ...
    return (
        <>
            <div className={`w-full h-auto flex flex-col justify-start items-center relative overflow-hidden bg-white`}>
                ...
                <div className={`w-full h-screen flex flex-col md:flex-row justify-start items-center md:py-3 md:px-3`}>
                    ...
                    <div className={`w-full ${otpSent ? "hidden" : "block"} px-5 sm:px-14 md:px-20 md:w-1/2 h-[65vh] sm:h-[55vh] md:h-auto pt-5 absolute md:static overflow-y-auto scroll-bar flex flex-col justify-start items-center bg-white bottom-0 rounded-t-[180px] md:rounded-none sm:rounded-t-[250px]`}>
                        <h1 className={`w-full px-10 text-center text-black font-genos text-2xl md:text-4xl pt-10`}>Create a new account</h1>

                        <input value={name} onChange={(e) => setName(e.target.value)} type="text" className={`w-full font-montserrat outline-none mt-8 text-black py-4 border-b-[2px] border-b-[#4a8808]`} placeholder="Enter fulll name*" />
                        <input value={email} onChange={(e) => setEmail(e.target.value)} type="text" className={`w-full font-montserrat outline-none py-4 text-black border-b-[2px] border-b-[#4a8808]`} placeholder="Enter email address*" />
                        <input value={contact} onChange={(e) => setContact(e.target.value)} type="text" className={`w-full font-montserrat outline-none py-4 text-black border-b-[2px] border-b-[#4a8808]`} placeholder="Enter contact number*" />
                        <div className={`w-full flex flex-col justify-center items-center gap-2 relative`}>
                            <input value={password} onChange={(e) => setPassword(e.target.value)} type={visible ? "text" : "password"} className={`w-full font-montserrat text-black outline-none py-4 border-b-[2px] border-b-[#4a8808]`} placeholder="Set a password*" />
