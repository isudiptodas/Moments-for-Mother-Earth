  ...
  return (
    <>
      <div className={`w-full z-40 ${dark ? "bg-zinc-900" : "bg-white"} duration-150 ease-in-out h-auto fixed flex justify-center md:justify-between items-center py-5 md:px-8 shadow-xl`}>
        ...
        <div className={`w-full h-auto z-40 px-5 rounded-b-3xl ${dark ? "backdrop-blur-3xl bg-black/60" : "backdrop-blur-3xl bg-white/25"} py-5 fixed top-0 ${visible ? "translate-y-0 shadow-2xl" : "-translate-y-full"} duration-200 ease-in-out flex flex-col justify-start items-start pt-16`}>
          ...
          <Link href='/user/blog' className={`w-full ${path === '/user/blog' ? "border-l-2 border-green-700 px-2 font-semibold" : ""} text-start font-montserrat text-lg py-1 my-2 ${dark ? "text-white" : "text-black"} duration-150 ease-in-out`}>Blog</Link>
        ...
        <div className={`w-auto z-40 hidden md:flex justify-center items-center gap-4 xl:gap-6`}>
          <Link href='/user/dashboard' className={`w-auto ${dark ? "text-white" : "text-black"} py-2 ${path === '/user/dashboard' ? "bg-gradient-to-r from-teal-400 to-green-700 rounded-full px-5 font-semibold" : ""} shrink-0 text-sm xl:text-lg font-montserrat`}>Home</Link>
          <Link href='/user/blog' className={`w-auto ${dark ? "text-white" : "text-black"} py-2 ${path === '/user/blog' ? "bg-gradient-to-r from-teal-400 to-green-700 rounded-full px-5 font-semibold" : ""} shrink-0 text-sm xl:text-lg font-montserrat`}>Blog</Link>
          <Link href='/user/podcast' className={`w-auto ${dark ? "text-white" : "text-black"} py-2 ${path === '/user/podcast' ? "bg-gradient-to-r from-teal-400 to-green-700 rounded-full px-5 font-semibold" : ""} shrink-0 text-sm xl:text-lg font-montserrat`}>Podcast</Link>
          <Link href='/user/events' className={`w-auto ${dark ? "text-white" : "text-black"} py-2 ${path === '/user/events' ? "bg-gradient-to-r from-teal-400 to-green-700 rounded-full px-5 font-semibold" : ""} shrink-0 text-sm xl:text-lg font-montserrat`}>Events</Link>
          <Link href='/user/donate' className={`w-auto ${dark ? "text-white" : "text-black"} py-2 ${path === '/user/donate' ? "bg-gradient-to-r from-teal-400 to-green-700 rounded-full px-5 font-semibold" : ""} shrink-0 text-sm xl:text-lg font-montserrat flex justify-center items-center gap-2`}>Donate <span className={`opacity-50 text-[10px] xl:text-sm`}>Beta</span></Link>
        </div>

        <div className={`w-auto z-40 hidden md:flex justify-center items-center gap-5`}>
          <Link href='/user/account' className={`w-auto ${path === '/user/account' ? "bg-gradient-to-r from-teal-400 to-green-700 rounded-full px-5 font-semibold" : ""} py-2 ${dark ? "text-white" : "text-black"} shrink-0 text-sm xl:text-lg font-montserrat`}>Account</Link>
          <input
            onChange={(e) => { setDark(!dark) }}
            type="checkbox"
            checked={dark}
